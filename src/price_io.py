...
price_dtype = np.dtype(
    [('symbol', np.str_, 10), ('date', 'datetime64[D]'), ('open', np.float32), ('high', np.float32), ('low', np.float32), ('close', np.float32), ('volume', np.float32), ('adj_close', np.float32)])
...
def upload_price_data(_data):
    ...
    tmp = db_io.cur()
    tmp.executemany(db_io.upload_price_stmt, _data)
    tmp.close()
    db_io.con().commit()
...
def update_price_many(_list=sp500[0:2]):
    for symbol in _list:
        try:
            update_price_data(symbol)
        except Exception as e:
            print(e)
"""
TESTING
