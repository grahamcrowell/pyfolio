...
rrsp_tran_dtype = np.dtype(
    [('desc', np.str_, 100), ('symbol', np.str_, 10), ('trans_date', np.datetime64), ('settle_date', np.datetime64), ('acct_currency', np.str_, 6), ('type', np.str_, 15), ('qty', np.float32),
...
def extract_date_rrsp(_dt_str):
    # 02-Jul-2014
    _format = "%d-%b-%Y"
    py_dt = datetime.datetime.strptime(_dt_str, _format).date()
...
if __name__ == '__main__':
    # db_io.remake_db()
    # ts = load_rrsp_csv_trans(rrsp_csv_file)
    # symbol_fill(ts)
    # data = ts.tolist()
    # upload_trans_data(data)
    # db_data = load_trans_data(_symbol=u'<cash>')
    # print(db_data)
    rebuild_trans_table()
    db_io.dump_db()
    db_io.clean_up()